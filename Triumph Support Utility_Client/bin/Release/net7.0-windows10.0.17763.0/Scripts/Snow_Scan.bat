...
call :sendSnowAgent
if %errorlevel% equ 0 (echo. & echo [EXITING: Error: Snow Agent Scan and Send encountered an issue] & exit) else (echo. & echo [EXITING: Successful: Snow Agent Scan and Send completed successfully] & exit)
...
snowagent.exe scan
if %errorlevel% equ 0 (echo # Snow Agent Scan successful!) else (echo. & echo # Snow Agent Scan unsuccessful! & timeout 5 & exit /b 1)
goto:eof
...
snowagent.exe send
if %errorlevel% equ 0 (echo # Snow Agent Scan sent successfully! & echo [%date% @ %time%] %COMPUTERNAME% Snow: Scanned, Sent >> "\\S-UKVM-FP15\grpgnrl\Daniel Mullings\Support Files\Scripts\Snow\SnowReinstallerLog.txt" & timeout 5 & exit /b 0) else (echo # Snow Agent Scan sent unsuccessful! & timeout 5 & exit /b 2)
goto:eof
